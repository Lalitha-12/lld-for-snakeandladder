    ...
    void startGame(){
        while(nextTurn.size()>1) {
            ...
            if (nextCell > boardSize) nextTurn.offer(player);
               ...
               if(nextPosition[0] != nextCell) System.out.println(player.getName() + " Got ladder present at: "+ nextCell);
                Ladder.forEach(v-> {
                    if(v.getStart()==nextCell) {
                        nextPosition[0] = v.getEnd();
                        b[0]=true;
                    } } );
                if(nextPosition[0] != nextCell && b[0]) System.out.println(player.getName() + " Bitten by snake at: "+ nextCell);
